        ...
        with open(file_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    # print(record)
                    score = record["fasttext_score"]
                    random_number = random.random()
                    threshold = 1-0.2*(1+score)

                    if score <= 0.5 and random_number >= threshold:
                        content = {}
                        content["text"] = record["text"]
                        content["id"] =  str(row)
                        content["source"] = tag + relative_path.replace('/', '#')
                        output_records.append(content)
                    #     print(f"record data with score {score}, random data {random_number} above equal threshold {threshold} ")
                    # else:
                    #     print(f"XXXXXXX  filter data with score {score} , data {random_number} above 0.5 or  below threshold {threshold} XXXXXX")
...
if __name__ == "__main__":
    ...
    root_directory = "/root/a100_nas/peixunban/001526_zdd/data/fasttext_scored_url_fitered/jsonl"
    filelist = "/root/a100_nas/peixunban/001526_zdd/data/fasttext_scored_url_fitered/jsonl/file_list"
    output_dir = "/root/a100_nas/peixunban/001526_zdd/data/dolma/fasttext_scored_url_fitered_dclm_all_below_0.5"
    output_file_list_path = "/root/a100_nas/peixunban/001526_zdd/data/dolma/fasttext_scored_url_fitered_dclm_all_below_0.5/file_list"


    # 确保输出目录存在
    os.makedirs(output_dir, exist_ok=True)

    main(root_directory, filelist, output_dir, output_file_list_path, useText = True, tag ="dclm_baseline_1.0_fasttext_scored_url_fitered_dclm_all_below_0.5#")
    end_time = time.time()
